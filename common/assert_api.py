...
class AssertApi():
    ...
    def assert_api(self, response, case, get_db=None):
        ...
        # 获取 ReadFile 实例并调用 write_config 方法
        read_file = get_readfile_instance()
        read_file.write_config(ExchangeData.get_extra_pool())  # 传递 obj 参数
    ...
    def assert_sql(self, case, get_db='true'):
        if get_db != None:
            sql_srt = case[-3]
            if sql_srt != "":
